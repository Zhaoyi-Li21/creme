...
# print(A.shape)
# torch.save(A, '/root/autodl-tmp/zhaoyi/knowledge_locate/rome/my_exps/cache/A.openalpaca.pt')
...
exp_name = 'new_overall_alpaca'
write_path = "/root/autodl-tmp/zhaoyi/knowledge_locate/debias_causal_intervent/datasets/"+exp_name+'.pre.json'
fw = open(write_path, "w")

data_path = "/root/autodl-tmp/zhaoyi/knowledge_locate/datasets/MQuAKE/datasets/new_debias/openalpaca.pre.groundtruth.json"
fr = open(data_path, "r")
data = json.load(fr)
...
W = mt.model.state_dict()['lm_head.weight'] # W * h = v, shape = [32000, hid_dim]
if use_t == False:
    A = torch.load('/root/autodl-tmp/zhaoyi/knowledge_locate/rome/my_exps/cache/A.openalpaca.pt').cuda() # A.shape = [hid_dim, 32000]
else:
