...
def calculate_hidden_flow(
    ...
    e_range = find_token_range(mt.tokenizer, inp["input_ids"][0], subject, mt.model_type)
...
def layername(model, num, kind=None):
    ...
    elif hasattr(model, "model"): # llama-family
        if kind == "embed":
            return "model.embed_tokens"
        if kind == "attn_head_q":
            return f'model.layers.{num}.self_attn.q_proj'
        if kind == "attn_head_k":
            return f'model.layers.{num}.self_attn.k_proj'
        if kind == "attn_head_v":
...
def plot_trace_heatmap(result, savepdf=None, title=None, xlabel=None, modelname=None, savefig=None):
...
def find_token_range(tokenizer, token_array, substring, model_type):
    toks = decode_tokens(tokenizer, token_array)
    print(toks, substring)
    if model_type == 'llama':
        sub_token_array = tokenizer.encode(substring)[1:] #detach the <sos> token
        print(token_array, sub_token_array)
        token_array = token_array.tolist()
        
        for i in range(0, len(token_array) - len(sub_token_array) + 1, 1):
            flag = False
            for j in range(0, len(sub_token_array)):
                if sub_token_array[j] != token_array[i+j]:
                    flag = True
                    break
            if flag == True:
                continue
            else:
                return (i, i+len(sub_token_array))
        raise Exception("Not Found Token Range for the LLaMA Model.")
