...
from utils.generate import generate_fast, generate_interactive, get_prob, get_key_probs
...
def test_rome(
    ...
    cloze_comp = request["prompt"].format(request["subject"])
    ...
    pre_prob_comp, pre_prob_wrong_comp\
        = get_key_probs(model_old, tokenizer, cloze_comp, template, explicit_toks, wrong_toks)
    ...
    # print_loud("Key possibilities for comp, guide and first-hop with the pre-update model")
    after_prob_comp, after_prob_wrong_comp\
        = get_key_probs(model_new, tokenizer,cloze_comp, template, explicit_toks, wrong_toks)
...
if __name__ == "__main__":
    ...
    # print(total_num)
    fr.close()
    write_path = read_path.replace('make_dataset', "results/rome")
