import json
check_key = "delta"
read_path = "/root/autodl-tmp/zhaoyi/knowledge_locate/FastEdit/results/v0/openalpaca.suf.json"
refer_path = read_path.replace('results/v2', 'make_dataset')
write_path = read_path.replace('suf','suf.pos_cnt.avg.'+check_key)
fr = open(read_path, "r")
    ...
    for key1 in key_list :
        for key2 in datum[key1][check_key]:
            if len(datum[key1][check_key][key2]) == 0:
                continue
            pos_cnt = 0
            for e in datum[key1][check_key][key2]:
                if e > 0: pos_cnt += 1
            
            out[key1][key2].append(pos_cnt/len(datum[key1][check_key][key2]))
            if key2 == "wrong":
